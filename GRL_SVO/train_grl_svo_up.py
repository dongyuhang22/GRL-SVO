...
from config import EnvT_config, model_config
import random
...
if __name__ == '__main__':
    ...
    train_list = os.listdir('datasets/dataset_train/polys')
    valid_list = os.listdir('datasets/dataset_valid/polys')

    # train result directory
    dir_basic = f'up_{train_time}'
    ...
    # to save train result and information
    ...
    def make_env(config: EnvT_config):
        def _thunk():
            env = gym.make('cad_order_train_up-v0', config=config)
            return env
        return _thunk
